...
def state_update_192(S, K, rounds):
    itr_cnt = rounds >> 5
...
def initialize_192(state, key, nonce):
    state = copy.deepcopy(state)

    state_update_192(state, key, 1152)

    for i in range(3):
        state[1] ^= FRAMEBITS_NONCE
        state_update_192(state, key, 640)
        nonce_word = struct.unpack("<I", nonce[i * 4: (i + 1) * 4])[0]
        state[3] ^= nonce_word

    return state

def process_associated_data_192(state, key, data):
    state = copy.deepcopy(state)
...
def process_plain_text_192(state, key, text, ct_len):
    part_byte_cnt = ct_len & 3
...
def process_cipher_text_192(state, key, cipher, ct_len):
    part_byte_cnt = ct_len & 3
...
def finalize_192(state, key, tag=None):
    state = copy.deepcopy(state)
...
def state_update_256(S, K, rounds):
    itr_cnt = rounds >> 5
...
def initialize_256(state, key, nonce):
    state = copy.deepcopy(state)

    state_update_256(state, key, 1280)

    for i in range(3):
        state[1] ^= FRAMEBITS_NONCE
        state_update_256(state, key, 640)
        nonce_word = struct.unpack("<I", nonce[i * 4: (i + 1) * 4])[0]
        state[3] ^= nonce_word

    return state

def process_associated_data_256(state, key, data):
    state = copy.deepcopy(state)
...
def process_plain_text_256(state, key, text, ct_len):
    part_byte_cnt = ct_len & 3
...
def process_cipher_text_256(state, key, cipher, ct_len):
    part_byte_cnt = ct_len & 3
...
def finalize_256(state, key, tag=None):
    state = copy.deepcopy(state)
